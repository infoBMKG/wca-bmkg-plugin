    ...
    class wcaBmkgUpdate
    {
        ...
        public function __construct()
        {
            ...
            add_filter("pre_set_site_transient_update_plugins", [
                $this,
                "update",
            ]); //Fix: Update hook that this not called that much often and make it compatible with autoupdate-feature
        ...
        public function request()
        {
            ...
            if (false === $remote || !$this->cache_allowed) {
                $remote = wp_remote_get(
                    "https://raw.githubusercontent.com/infoBMKG/wca-bmkg-plugin/main/plugin-update.json?cache=" .
                        rand(10, 100),
                    [
                        "timeout" => 10,
                        "headers" => [
...
function wca_list_posts($param)
{
    ...
    foreach ($posts as $post) {
        ...
        $data[$i]["excerpt"] = apply_filters(
            "the_excerpt",
            $post->post_excerpt
        );
        $data[$i]["featured_image"]["thumbnail"] = get_the_post_thumbnail_url(
            $post->ID,
            "thumbnail"
        );
        ...
        $data[$i]["featured_image"]["full"] = get_the_post_thumbnail_url(
            $post->ID,
            "full"
        );
        $i++;
...
function wca_posts($slug)
{
    ...
    $data["author"] = get_the_author_meta(
        "display_name",
        $post[0]->post_author
    );
    ...
    $data["featured_image"]["full"] = get_the_post_thumbnail_url(
        $post[0]->ID,
        "full"
    );
...
add_action("rest_api_init", function () {
    ...
    register_rest_route("wca/v1", "posts/(?P<slug>[a-zA-Z0-9-]+)", [
        "methods" => "GET",
        "callback" => "wca_posts",
        // 'args' => array(
        // 	'slug' => array(
    ...
    register_rest_route("wca/v1", "pages/(?P<slug>[a-zA-Z0-9-]+)", [
        "methods" => "GET",
        "callback" => "wca_pages",
        // 'args' => array(
        // 	'slug' => array(
