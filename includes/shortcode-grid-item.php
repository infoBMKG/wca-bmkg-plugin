 ...
 * Menampilkan item dalam layout grid 1/3 (gambar) + 2/3 (teks) di layar medium+.
 * Deskripsi dimasukkan di antara tag shortcode.
 *
 * @param array $atts Atribut shortcode (image_src, image_alt, title, link_url, link_text).
 * @param string|null $content Konten deskripsi yang diapit oleh tag shortcode.
...
function bmkg_grid_item_shortcode_handler($atts, $content = null)
{
    ...
    // Validasi atribut wajib & konten deskripsi
    if (
        ...
        // Return kosong jika ada yang kurang
        return "";
    }

    // Sanitasi & Escape atribut
    ...
    // Rakit HTML output menggunakan HEREDOC
    $output = <<<HTML
<div class="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-3 gap-0 md:gap-8 mb-6 md:mb-8">
    <div class="md:col-span-1">
      <img class="w-full h-auto size-full rounded-lg object-cover" src="{$image_src}" alt="{$image_alt}" />
    </div>
    <div class="md:col-span-2">
      <h3 class="text-lg xl:text-2xl font-bold mb-4">{$title}</h3>
      <div class="prose prose-sm max-w-none">{$processed_content}</div>
      <a href="{$link_url}" target="_blank" rel="noopener noreferrer" class="mt-3 inline-flex justify-start items-center text-blue-primary text-base font-medium hover:underline">
        {$link_text}
